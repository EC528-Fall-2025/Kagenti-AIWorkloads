...
We contributed to the larger open-source Kagenti project by creating a variety of agent and MCP server examples that can be integrated into the Kagenti platform, as well as making Pull Requests to improve the overall platform as a whole.
...
Finally, we wrote a developer's guide on how to develop A2A Agents for the Kagenti's Medium Blog. While it is undergoing final review by the Kagenti Team before the publishing stage, the completed submitted draft can be found [here](https://docs.google.com/document/d/16-AwtMizRZ-Opl-PSb_5Nq45cGAPGW44RIdsroZX0eU/edit?usp=sharing).

## Installation and Setup

To install and run the Kagenti platform see [this document](https://github.com/kagenti/kagenti/blob/main/docs/kind-install.md). After installing, this [quickstart guide](https://github.com/kagenti/kagenti/blob/main/docs/demo-weather-agent.md) demonstrates how to deploy your first agent and tool. 

Documentation for the specific agents and MCP servers we developed can be found below.

## Agents and MCP servers

- **Generic Agent** (with Flight and Movie MCP Servers) [(PR)](https://github.com/kagenti/agent-examples/pull/93): This is a multi-purpose agent that can connect to multiple MCP servers and selects from multiple tools based on the task. To run this with our flight and movie tools, see [this demo](https://github.com/SiennaChien/kagenti-fork/blob/generic_docs/docs/demo-generic-agent.md). It can be set up with these MCP servers:
...
## General Platform Contributions

Throughout our submitted PR's, we have added multiple features and improvements to the platform. We have included descriptions on a few notable ones as well as how to run them in this section.

### Kagenti Dependencies Helm Chart [(PR)](https://github.com/kagenti/kagenti/pull/279)
...
### Kagenti Operator Helm Chart [(PR)](https://github.com/kagenti/kagenti-operator/pull/108)
...
### MCP Gateway Tool Annotations and Observability Stack [(PR)](https://github.com/kagenti/mcp-gateway/pull/311)
...
The project aims to expand the capabilities and adoption of [Kagenti](https://github.com/kagenti) by defining and implementing new use cases for the platform. This includes extending the core platform to support these new applications.
...
- **New Use Case Development**: We will design, build, and document new use cases for the Kagenti platform. This involves:
  - **Developing or Modifying AI Agents and Tools**: We will create or adapt agents to solve real-world problems. For example, we might develop an agent that integrates with cloud storage systems to automate file organization.
...
- **Platform Feature Developmen (all components)**: We will be building contributing directly via GitHub pull-requests into the Kagenti core platform (MCP gateway, operator) wherever the priorities of the IBM team lie. This includes but is not limited to fixing bugs, adding features, or developing scripts for installation.

### Out of Scope

- **Deep Technical Modifications to Existing Agents**: While we may modify agents to suit our use cases, we will not be undertaking major refactoring or deep technical work on the codebase of pre-existing agents outside the scope of our project. This includes integrating higher-level security features such as token exchange and workload identity
